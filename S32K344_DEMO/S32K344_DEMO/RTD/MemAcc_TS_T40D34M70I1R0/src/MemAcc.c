...
#define MEMACC_AR_RELEASE_MINOR_VERSION_C       9
#define MEMACC_AR_RELEASE_REVISION_VERSION_C    0
#define MEMACC_SW_MAJOR_VERSION_C               7
#define MEMACC_SW_MINOR_VERSION_C               0
#define MEMACC_SW_PATCH_VERSION_C               1
...
/*! to void CERT ARR32-C, MISRA C-2012 Rule 18.8 violations this macro defined as maximum number of instances so total mem driver have
 - mem_infls 1, Mem_Exfls 1, Mem_eep 1, mem_mram 1 so maximun number instance can config as 4
 */
#define MAX_HW_RESOURCE_COUNT                           (4U)
/*==================================================================================================
*                                         LOCAL CONSTANTS
==================================================================================================*/
 ...
 */
static void MemAcc_ReportDevError(uint8 ApiId,
                                  uint8 ErrorId
                                 )
{
    /* Raise the development error via Det function */
    /* There is no instance in Mem Acc so InstanceId is 0U*/
    (void)Det_ReportError(MEMACC_MODULE_ID, (uint8)0U, ApiId, ErrorId);
}
...
static void MemAcc_InitMemDrivers(void)
{
    ...
    {
        if (MEMACC_INDIRECT_STATIC == MemAcc_pConfigPtr->MemApisInvocation[Index])
        {
...
static MemAcc_LengthType MemAcc_GetPhysicalSegmentSize(const MemAcc_JobRuntimeInfoType *JobRequest)
{
    ...
    switch (JobRequest->JobType)
    {
        ...
        case MEMACC_BLANKCHECK_JOB:
            PhysicalSegmentSize = JobRequest->SubArea->MemSectorBatchInfo.ReadMinSize;
            break;
 ...
 */
static inline uint8 MemAcc_ValidateModuleInitialized(void)
{
    uint8 ErrorId = MEMACC_E_OK;
 ...
 */
static uint8 MemAcc_ValidateAlignment(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint8 ErrorId = MEMACC_E_OK;
    MemAcc_LengthType PhysicalSegmentSize;
 ...
 */
static uint8 MemAcc_ValidateRange(const MemAcc_JobRuntimeInfoType *JobRequest,
 ...
 */
static inline uint8 MemAcc_ValidateDataPointer(const MemAcc_DataType *DataPtr)
{
    uint8 ErrorId = MEMACC_E_OK;
 ...
 */
static uint8 MemAcc_ValidateMemDriverBinaryDelimiterField(const MemAcc_MemDriverHeaderType  *MemDriverHeaderPtr)
{
    uint8 ErrorId = MEMACC_E_OK;
 ...
 */
static uint8 MemAcc_ValidateMemDriverBinary(MemAcc_AddressType HeaderAddress,
                                             MemAcc_HwIdType HwId
                                            )
{
    uint8 ErrorId = MEMACC_E_OK;
    const MemAcc_MemDriverHeaderType  *MemDriverHeader;

    if (HeaderAddress > 0U)
    {
        MemDriverHeader = (MemAcc_MemDriverHeaderType *)((MemAcc_UintPtrType)HeaderAddress);
    }
    else
    {
        ErrorId = MEMACC_E_MEM_INIT_FAILED;
 ...
 */
static uint8 MemAcc_ValidatePartition(void)
{
    uint8 ErrorId = MEMACC_E_OK;
    uint8  UserId;
 ...
 */
static uint8 MemAcc_ValidateAddressAreaAndPartition(uint16 AreaIndex)
{
    uint8 ErrorId = MEMACC_E_OK;
    uint8  UserId;
 ...
 */
static uint8 MemAcc_ValidateAddressAndLength(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint8 ErrorId;
    const MemAcc_AddressAreaType *AddressArea = &(MemAcc_pConfigPtr->AddressAreas[JobRequest->AreaIndex]);
 ...
 */
static uint8 MemAcc_SetParamJobCommon(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint8 ErrorId;
 ...
 */
static uint8 MemAcc_SetParamJobDataTransfer(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint8 ErrorId;
 ...
 */
static uint8 MemAcc_ConfigureJobRequest(MemAcc_JobRuntimeInfoType *JobRequest)
{
    uint8 ErrorId;
    ...
    else
    {
        ...
        JobRequest->CurrentConsolidatedResult = MEMACC_OK;
    ...
    if (MemAcc_pConfigPtr->AddressAreas[AreaRequesting].Priority > MemAcc_pConfigPtr->AddressAreas[AreaExecuting].Priority)
    {
        ...
        if (MEMACC_JOB_STATE_RETRYING == JobExecuting->JobState)
        {
            ...
            else
            {
                ...
                JobExecuting->JobResult = MEMACC_FAILED;
            }
            /*The execute job has no pending request on Mem driver, the hw resource will be locked by the requesting job now.*/
            MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResources = AreaRequesting;
...
static boolean MemAccCheckHwResourceLocked(const MemAcc_JobRuntimeInfoType *JobInfo, boolean *HwResourceGranted)
{
    ...
    uint16 AreaExecuting        = MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResources;  /* The address area is using the hardware resource */

#if (STD_ON == MEMACC_MULTI_PARTITION_TYPE_1_ENABLED)
    Std_ReturnType RetVal;

    /* Check the sema4 is locked by current core or not*/
#if (STD_ON == MEMACC_MULTI_PARTITION_ADVANCE_ENABLED)
    if (MemAcc_pConfigPtr->SemaphoreConfig[MemHwResource].AdvancedModeEnable == TRUE)
    {
        RetVal = MemAcc_Sema4_IsLockedAdvancedMode(JobInfo);
    }
    else
#endif
    {
        RetVal = MemAcc_Sema4_IsLocked(MemHwResource);
    }
    if (E_OK != RetVal)
    {
        /*Clear lock by the variable, need to try lock again*/
        MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResourcesLocked = MEMACC_HW_RESOURCE_IDLE_U16;
        MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResources     = MEMACC_HW_RESOURCE_IDLE_U16;
    ...
    if (MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResourcesLocked != MEMACC_HW_RESOURCE_IDLE_U16)
    {
...
static boolean MemAcc_RequestMemHwResource(const MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    AreaExecuting = MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResources;  /* The address area is using the hardware resource */
    ...
    else if (MEMACC_HW_RESOURCE_IDLE_U16 == AreaExecuting)
    {
#if (STD_ON == MEMACC_MULTI_PARTITION_TYPE_1_ENABLED)
        ...
        if (MemAcc_pConfigPtr->SemaphoreConfig[MemHwResource].AdvancedModeEnable == TRUE)
        {
            RetVal = MemAcc_Sema4_RequestLockAdvancedMode(JobInfo);
        }
        else
#endif /* MEMACC_MULTI_PARTITION_ADVANCE_ENABLED */
        {
            RetVal = MemAcc_Sema4_RequestLock(MemHwResource);
        }
...
#endif /* MEMACC_MULTI_PARTITION_TYPE_1_ENABLED */
        {
            ...
            {
                /*Set lock resource*/
                MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResourcesLocked = AreaRequesting;
            }
        }
    }
    else
...
static void MemAcc_ReleaseMemHwResource(const uint8 MemHwResource)
{
    ...
    /* Release the hardware resource in multi partition mode */
        ...
        if (MemAcc_pConfigPtr->SemaphoreConfig[MemHwResource].AdvancedModeEnable == TRUE)
        {
            MemAcc_Sema4_ReleaseLockAdvancedMode(MemHwResource);
        }
        else
#endif /* MEMACC_MULTI_PARTITION_ADVANCE_ENABLED */
        {
...
static void MemAcc_CalculateLengthTransfer(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    switch (JobInfo->JobType)
    {
        ...
        /* fall-through */
        case MEMACC_BLANKCHECK_JOB:
            PhysicalSegmentSize = JobInfo->SubArea->MemSectorBatchInfo.ReadMaxSize;
            break;
#if (MEMACC_COMPARE_API == STD_ON)
        case MEMACC_COMPARE_JOB:
            PhysicalSegmentSize = JobInfo->SubArea->MemSectorBatchInfo.ReadMaxSize;
...
static void MemAcc_JobRequestMemService(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    else
    {
        ...
        if (JobInfo->JobRetries == 0U)
        {
            ...
            JobInfo->JobResult = MEMACC_FAILED;
        }
        else
        {
            /* Reduce the number of remaining retries */
                                             ...
                                             )
{
    switch (MemJobResult)
    {
        ...
        case MEM_INCONSISTENT:
            JobInfo->JobResult = MEMACC_INCONSISTENT;
            break;
        case MEM_ECC_UNCORRECTED:
            JobInfo->JobResult = MEMACC_ECC_UNCORRECTED;
            break;
        default:
            /* Do nothing */
            break;
...
static void MemAcc_JobLocking(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    uint16                              MemHwResource       = MEMACC_HW_RESOURCE_IDLE_U16;
    uint32                              EndAddress          = JobInfo->LogicAddress + JobInfo->LengthOrigin;
    const MemAcc_SubAddressAreaType     *SubAreaWillBeLocked[MAX_HW_RESOURCE_COUNT];
    const MemAcc_AddressAreaType        *AddressArea;
    uint16                              SubAreaIndex;
    uint8                               HwResource;
    uint8                               UnlockResourceIndex;

    /* Init the array the pointers to Sub-Areas including the Hardware resource information will be locked */
    for (HwResource = 0; HwResource < MAX_HW_RESOURCE_COUNT; HwResource++)
    {
    ...
    for (HwResource = 0; HwResource < MemAcc_pConfigPtr->HwResourceCount; HwResource++)
    {
    ...
    if (HwResourceGranted == FALSE)
    {
        ...
        /* Not unlock when UnlockResourceIndex = HwResource  because it is current Resource lock fails */
        for (UnlockResourceIndex = 0; UnlockResourceIndex < HwResource; UnlockResourceIndex++)
        {
            /* Perform unlocking of all HwResources from the first locked #HwResource to latest #HwResource that previously failed to lock */
            /* Check if hardware resource that was locked before by current AreaIndex */
            if (MemAcc_pConfigPtr->HwResourcesStatus[UnlockResourceIndex].MemHwResourcesLocked == JobInfo->AreaIndex)
            {
                /* Perform unlock */
                MemAcc_ReleaseMemHwResource(UnlockResourceIndex);
                MemAcc_pConfigPtr->HwResourcesStatus[UnlockResourceIndex].MemHwResourcesLocked = MEMACC_HW_RESOURCE_IDLE_U16;
            }
        }
...
static void MemAcc_JobProcessing(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    switch (MemJobResult)
    {
        ...
        case MEM_ECC_CORRECTED:
            /* final job is MEMACC_ECC_CORRECTED if any job is MEMACC_ECC_CORRECTED to match the SWS_MemAcc_00092 MEMACC_ECC_CORRECTED */
            JobInfo->CurrentConsolidatedResult = MEMACC_ECC_CORRECTED;
...
static void MemAcc_JobSuspending(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    switch (MemJobResult)
    {
        case MEM_JOB_OK:
            ...
            break;
        case MEM_ECC_CORRECTED:
             /* final job is MEMACC_ECC_CORRECTED if any job is MEMACC_ECC_CORRECTED to match the SWS_MemAcc_00092 */
            JobInfo->CurrentConsolidatedResult = MEMACC_ECC_CORRECTED;
            MemAcc_JobSuspendingOnSuccess(JobInfo);
            break;
        case MEM_JOB_PENDING:
...
static void MemAcc_JobCanceling(MemAcc_JobRuntimeInfoType *JobInfo)
{
    ...
    switch (MemJobResult)
    {
        ...
        case MEM_ECC_CORRECTED:
            ...
            JobInfo->JobState = MEMACC_JOB_STATE_STOP;
            JobInfo->JobStatus = MEMACC_JOB_IDLE;
            JobInfo->JobResult = MEMACC_CANCELED;
            break;
        case MEM_JOB_FAILED:
        /* fall-through */
        case MEM_INCONSISTENT:
...
{
    if (MEMACC_DIRECT_STATIC != JobInfo->SubArea->MemInvocation)
    {
    ...
    else
    {
        ...
        if ((Std_ReturnType)E_OK == Status)
        {
...
#endif
            {
                ...
                {
                    MemAcc_pJobRuntimeInfo[Index].CurrentConsolidatedResult = MEMACC_OK;
                    MemAcc_pJobRuntimeInfo[Index].JobResult = MEMACC_OK;
                ...
                for (Index = 0U; Index < MemAcc_pConfigPtr->HwResourceCount; Index++)
                {
                    MemAcc_pConfigPtr->HwResourcesStatus[Index].MemHwResources = MEMACC_HW_RESOURCE_IDLE_U16;
                    MemAcc_pConfigPtr->HwResourcesStatus[Index].MemHwResourcesLocked = MEMACC_HW_RESOURCE_IDLE_U16;
                }
            }
        }
        else
...
void MemAcc_DeInit(void)
{
    ...
    for (MemHwResourcesIndex = 0; MemHwResourcesIndex < MemAcc_pConfigPtr->HwResourceCount; MemHwResourcesIndex++)
    {
        /* Release the hardware resource in multi partition mode */
...
void MemAcc_GetVersionInfo(Std_VersionInfoType * VersionInfoPtr)
{
    ...
    {
        /* Report the error, the VersionInfoPtr argument is inconsistent */
...
MemAcc_JobResultType MemAcc_GetJobResult(MemAcc_AddressAreaIdType AddressAreaId)
{
    ...
    JobResult = MEMACC_FAILED;

    /* Checks if the MemAcc module has been initialized */
    uint8 ErrorId = MemAcc_ValidateModuleInitialized();
                                   ...
                                   )
{
...
#endif /* MEMACC_DEV_ERROR_DETECT == STD_ON */
    {
        ...
        else
        {
            ...
            MemoryInfoPtr->ReadMinSize          = SubAreaInfo->MemSectorBatchInfo.ReadMinSize;      /* Smallest readable unit in bytes                                                    */
            MemoryInfoPtr->WritePageSize        = SubAreaInfo->MemSectorBatchInfo.WritePageSize;   /* Write size of a page in bytes                                                      */
            MemoryInfoPtr->ReadMaxSize          = SubAreaInfo->MemSectorBatchInfo.ReadMaxSize;     /* Largest readable unit in bytes                                                     */
                                 ...
                                 )
{
    ...
    const MemAcc_MemBinaryHeaderType *pMemApis = NULL_PTR;
#if (MEMACC_DEV_ERROR_DETECT == STD_ON)
    uint8 ErrorId;
#endif
...
#endif /* MEMACC_DEV_ERROR_DETECT == STD_ON */
    {
        ...
        if (MemAcc_pConfigPtr->MemApisInvocation[HwId] == MEMACC_INDIRECT_DYNAMIC)
        {
            if (HeaderAddress > 0U)
            {
                pMemApis = (MemAcc_MemBinaryHeaderType *)((MemAcc_UintPtrType)HeaderAddress);
            ...
            {
                RetVal = (Std_ReturnType)MEMACC_E_MEM_INIT_FAILED;
            }
        }
    }
                                   ...
                                   )
{
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        else
        {
            ...
            {
                /* Check HwId is correct for the Mem driver which is INDIRECT_DYNAMIC */
                if (MEMACC_INDIRECT_DYNAMIC == MemAcc_pConfigPtr->MemApisInvocation[HwId])
                {
 ...
 *                   If the read operation was successful, the result of the job is MEMACC_OK.
 *                   If the read operation failed, the result of the job is either MEMACC_FAILED in case of a general error
 *                   or MEMACC_ECC_CORRECTED/MEMACC_ECC_UNCORRECTED in case of a correctable/uncorrectable ECC error.
 *
 * @param[in]        AddressAreaId         Numeric identifier of address area.
 ...
 *                   The result of this service can be retrieved using the MemAcc_GetJobResult API.
 *                   If the write operation was successful, the job result is MEMACC_OK.
 *                   If there was an issue writing the data, the result is MEMACC_FAILED.
 *
 * @param[in]        AddressAreaId         Numeric identifier of address area.
 ...
 *                   The result of this service can be retrieved using the MemAcc_GetJobResult() API.
 *                   If the compare operation determined a mismatch, the result code is MEMACC_INCONSISTENT.
 *
 * @param[in]        AddressAreaId         Numeric identifier of address area.
 ...
 *                   The result of this service can be retrieved using the MemAcc_GetJobResult API.
 *                   If the address area defined by targetAddress and length is blank, the result is MEMACC_OK,
 *                   otherwise the result is MEMACC_INCONSISTENT.
 *
 * @param[in]        AddressAreaId         Numeric identifier of address area.
 ...
 *                   The result of this service can be retrieved using the MemAcc_GetJobResult API.
 *                   If the hardware specific operation was successful, the result of the job is MEMACC_OK.
 *                   If the hardware specific operation failed, the result of the job is MEMACC_FAILED.
 *
 * @param[in]        AddressAreaId         Numeric identifier of address area.
 * @param[in]        HwId                  Unique numeric memory driver identifier.
    ...
    if (MEMACC_E_OK == ErrorId)
    {
...
#endif
        {
            ...
            if((uint32)MEMACC_ADDRESS_AREA_INDEX_INVALID_U32 != AreaIndex)
            {
                ...
                {
                    if (MemAcc_pConfigPtr->HwResourcesStatus[SubAreaPtr->MemHwResource].MemHwResourcesLocked != MEMACC_HW_RESOURCE_IDLE_U16)
                    {
                                 ...
                                 )
{
    ...
    uint8 HwResource;
    uint8 ErrorId;
    boolean HwResourceWillBeUnlocked[MAX_HW_RESOURCE_COUNT];
    const MemAcc_AddressAreaType *AddressArea;
    uint16 SubAreaIndex;
    ...
    if (MEMACC_E_OK == ErrorId)
    {
        ...
        do
        {
            if ((&JobRequest)->SubArea->MemHwResource != MemHwResource)
            {
                ...
                if (MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResourcesLocked == (&JobRequest)->AreaIndex)
                {
                    HwResourceWillBeUnlocked[MemHwResource] = TRUE;
                }
                else if (MemAcc_pConfigPtr->HwResourcesStatus[MemHwResource].MemHwResourcesLocked != MEMACC_HW_RESOURCE_IDLE_U16)
                {
        ...
        if (RetVal == E_OK)
        {
            ...
            for (HwResource = 0;HwResource < MemAcc_pConfigPtr->HwResourceCount; HwResource++)
            {
                if (HwResourceWillBeUnlocked[HwResource] == TRUE)
                {
                    ...
                    MemAcc_pConfigPtr->HwResourcesStatus[HwResource].MemHwResourcesLocked = MEMACC_HW_RESOURCE_IDLE_U16;
                }
            }
        }
